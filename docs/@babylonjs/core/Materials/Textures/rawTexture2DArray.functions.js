...
import { _IsSideEffectImplemented, _WarnImport } from "../../Misc/devTools.js";
 ...
 */
export function UploadImageToTexture2DArrayLayer(texture, source, layer, options) {
 ...
 */
export async function LoadImageToTexture2DArrayLayerAsync(texture, url, layer, options) {
 ...
 */
export async function CreateTexture2DArrayFromKTX2Async(scene, data, options) {
    let buffer;
    ...
    const { KhronosTextureContainer2 } = await import("../../Misc/khronosTextureContainer2.js");
    ...
    for (const mipmap of baseLevel) {
        ...
        if (mipmap.width !== width || mipmap.height !== height) {
            throw new Error(`Failed to decode the KTX2 file: layer ${mipmap.layerIndex} of the base mip level is ${mipmap.width}x${mipmap.height} but layer 0 is ${width}x${height}.`);
        }
        if (mipmap.data.byteLength !== expectedLayerByteLength) {
            throw new Error(`Failed to decode the KTX2 file: layer ${mipmap.layerIndex} of the base mip level holds ${mipmap.data.byteLength} bytes but ${expectedLayerByteLength} were expected.`);
        }
    }
    ...
    return new RawTexture2DArray(textureData, width, height, layerCount, 5, scene, options?.generateMipMaps ?? true, options?.invertY ?? false, options?.samplingMode, 0);
}
/**
 ...
 * @returns a promise resolved with the created RawTexture2DArray
 */
export async function CreateTexture2DArrayFromImageUrlsAsync(scene, urls, options) {
    ...
    const bitmaps = results.filter((result) => result.status === "fulfilled").map((result) => result.value);
    try {
        ...
        const texture = new RawTexture2DArray(null, width, height, bitmaps.length, 5, scene, options?.generateMipMaps ?? true, options?.invertY ?? false, options?.samplingMode, options?.textureType);
        try {
            const uploadOptions = {
...
//# sourceMappingURL=rawTexture2DArray.functions.js.map