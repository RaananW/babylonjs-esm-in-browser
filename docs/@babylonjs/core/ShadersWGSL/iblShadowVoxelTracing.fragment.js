...
var invD: vec3f=ray_vs.dir_rcp;var D: vec3f=ray_vs.dir;var O: vec3f=ray_vs.orig;var negD=vec3i(lessThan(D, vec3f(0,0,0)));var voxel0: i32=negD.x | (negD.y<<1) | (negD.z<<2);var t0: vec3f=-O*invD;var t1=(vec3f(1.0)-O)*invD;var maxLod: i32= i32(highestMipLevel);var stackLevel: i32=0;var leafTests: i32=0;
#if VOXEL_MARCH_DIAGNOSTIC_INFO_OPTION
var steps: u32=0u;
#endif
...
vec4i(elem & 0xFF,(elem>>8) & 0xFF,(elem>>16) & 0xFF,elem>>24);if (Coords.w==0) {let cellOpacity: f32=textureLoad(voxelGridSampler,Coords.xyz,0).x;if (cellOpacity>=1.0) {
#if VOXEL_MARCH_DIAGNOSTIC_INFO_OPTION
...
#endif
return false;}
leafTests++;if (cellOpacity>=prngCanonical3d(vec3f(Coords.xyz)+rouletteSeed)) {
#if VOXEL_MARCH_DIAGNOSTIC_INFO_OPTION
*voxel_march_diagnostic_info.heat= f32(steps)/24.0;
#endif
return true;}
...
ray_vs.t_min=max(ray_vs.t_min,near);ray_vs.t_max=min(ray_vs.t_max,far);let rouletteSeed: vec3f=vec3f(DitherNoise,DitherNoise.x+DitherNoise.y)*17.0;
#if VOXEL_MARCH_DIAGNOSTIC_INFO_OPTION
