...
import { Vector3, Vector2, TmpVectors, Vector4 } from "../Maths/math.vector.pure.js";
...
export class GroundMesh extends Mesh {
    ...
    _computeHeightQuads() {
        ...
        let j;
        let k;
        let cd; // 2D slope coefficient : z = cd * x + h
        let h;
        let d1; // facet plane equation : ax + by + cz + d = 0
        let d2;
        const subdivisionsX = this._subdivisionsX;
        const subdivisionsY = this._subdivisionsY;
        for (let row = 0; row < subdivisionsY; row++) {
 ...
 */
export function RegisterGroundMesh() {
    if (_Registered) {
        return;
    }
    _Registered = true;
...
//# sourceMappingURL=groundMesh.pure.js.map